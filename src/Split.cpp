...
std::vector<Route> Split::splitLinear(Individual&individual)
{
    ...
    for(int j = 0; j <= static_cast<int>(this->individual.chromosome.size()); j++) {
        for(int i = j-1; i >= 0; i--) {
            ...
            if(distanceCumulative < this->cumulative_distance[(unsigned)j]) {
                ...
                bestRoute[(unsigned)j] = dataRoute.value();
            }
        }
    ...
    while(end > 1) {
        ...
        routesFinals[(unsigned)x].customers = std::vector<int>(bestRoute[(unsigned)end].route.size());
        routesFinals[(unsigned)x].assigned_lockers = std::vector<int>(bestRoute[(unsigned)end].route.size());
        routesFinals[(unsigned)x].total_distance = bestRoute[(unsigned)end].distance;
        routesFinals[(unsigned)x].load = bestRoute[(unsigned)end].load;
        for(int i = 0; i < static_cast<int>(bestRoute[(unsigned)end].route.size()); i++) {
            // std::cout << this->individual.chromosome[init+i] << " ";
            routesFinals[(unsigned)x].customers[(unsigned)i] = bestRoute[(unsigned)end].customers[(unsigned)i];
            // std::cout << "cliente visitado: " << routesFinals[x].customers[i] << std::endl;
            if(bestRoute[(unsigned)end].route[(unsigned)i] > 0) {
                ...
                routesFinals[(unsigned)x].assigned_lockers[(unsigned)i] = -1;
            } else {
                // std::cout << "eh locker" << std::endl;
                routesFinals[(unsigned)x].assigned_lockers[(unsigned)i] = abs(bestRoute[(unsigned)end].route[(unsigned)i]);
            }
    ...
    for(int x = i; x < j; x++) { 
        ...
        if(locationChosen >= 0 && x < j-1) { // cliente ou deposito ultmo visitado
            ...
            if(locker.has_value()) {
                ...
                if(verifyBestLocker(locker.value(),prev,cumulative_cost,distanceTemporary)) { // se for true entao eh melhor adicionar no locker

                    locationChosen = static_cast<int>(-(locker->id));
                    dataRoute->route.push_back((int)locationChosen);
                    dataRoute->route.push_back((int)locationChosen);
                    dataRoute->lockers.insert((int)locker->id);
        ...
        if(fleetTime > clientA.time_window.end && useClient) {
            ...
            locationChosen = static_cast<int>(-(lockerAdd->id));
...
bool Split::verifyBestLocker(Locker&locker, Point&prev, double cumulative_cost, double cumulativeDistance)
{
