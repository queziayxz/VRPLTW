...
int main(int argc, char** argv)
{
  ...
  // std::cout << "caminho main: " << src_dir << std::endl;

  if(argc == 2) {
    std::cout << "ler todas" << std::endl;
    resultAllInstances();
  } else if(argc == 3) {
    std::cout << "ler unica" << std::endl;
    std::string all = argv[1];
    std::string name = argv[2];

    if(all == "1") {
      std::cout << "Executando Algoritmo para " << name << std::endl;
      resultOnlyInstance(name);
    }
  } else {
